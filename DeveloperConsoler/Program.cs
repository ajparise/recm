    ...
    class Program
    {
        static void Main(string[] args)
        {
           ...
           // monitor.StatusMessage += new REConnectionMonitor.OnStatusMessage(monitor_StatusMessage);
            ...
            RecmDataContext db = new RecmDataContext();
            var tlsys = db.LockConnections_AllActiveREConnections_ClientOnly;
            ...
            var connections = tlsys;// db.LockConnections_AllActiveREConnectionsAliveOnly_ClientOnly.ToList().OrderByDescending(a => a.REProcess.IdleTime.TotalMilliseconds);
            ...
            {
                Console.WriteLine("\n{0} -- {1} -- {2}", c.Lock.HostName, c.Lock.User.Name, c.AllProcesses.FirstOrDefault() != null ? c.AllProcesses.FirstOrDefault().hostname.Trim() : "N/A (Dead Lock)");
                foreach (var p in c.AllProcesses)
                {
                    Console.WriteLine("\t{3} -- {0} -- {1} -- {2}",
                       p.spid,
                       p.program_name.Trim(),
